...
#  | -overhaul:  Only covers users who have completely updated their repo using the appropriate command
 ...
 curl -XPOST -u $INFLUXDB_ADMIN_USER:$INFLUXDB_ADMIN_PASSWORD  http://$URL_BASE:8086/query --data-urlencode 'q=CREATE DATABASE "cluster_actions"'
 ...
 curl -XPOST -u $INFLUXDB_ADMIN_USER:$INFLUXDB_ADMIN_PASSWORD  http://$URL_BASE:8086/query \
      --data-urlencode "q=GRANT WRITE ON \"cluster_actions\" TO \"$INFLUXDB_WRITE_USER\""
